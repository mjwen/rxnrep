...
class RxnRepLightningModel(pl.LightningModule):
    def __init__(self, params):
        ...
        if self.backbone.hparams.reaction_conv_layer_sizes:
            conv_last_layer_size = self.backbone.hparams.reaction_conv_layer_sizes[-1]
        # pretrained model does not have reaction conv layer
        else:
...
def main():
    ...
    if args.restore and log_save_dir.exists():
        # restore
        checkpoint_path = load_checkpoint_wandb(log_save_dir, project)
    else:
        # create new
