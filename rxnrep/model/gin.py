...
class GINConvOriginal(nn.Module):
    """
    The protein function prediction model as in:
    Strategies for pretraining graph neural networks.

    This is the original GINConv, with edge features concatenated to bond features and
    do not perform edge update explicitly.

    Note, we do not use self-loop edge as in the paper, because such edge cannot be
    featurized. We do use self-loop atom nodes.
...
class GINConv(nn.Module):
    ...
    This is modified GIN, where we do bond feature update explicitly.

    Note, we do not use self-loop edge as in the paper, because such edges cannot be
    easily featurized. We do use self-loop atom nodes.
